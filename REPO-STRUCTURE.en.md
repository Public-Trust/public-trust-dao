...
| `web/` | Public static showcase site (GitHub Pages). | `index.html`, local assets (`assets/`), `.nojekyll`. No external requests/CDN/trackers. | Build junk, keys, surveillance analytics. |
| `platform/` | Full web platform on Next.js (the application for people, product layer). Static export (`next export`). | `app/` (screens), `components/`, `lib/` (bilingual texts), `package.json` (exact versions), `next.config.mjs`. No external trackers/CDN/fonts. | `node_modules/`, `.next/`, `out/` (in `.gitignore`), keys, surveillance analytics. |
