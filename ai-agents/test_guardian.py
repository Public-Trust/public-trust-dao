...
def main():
    ...
    # 12. Отпечаток журнала, подписанный по-русски (CHANGELOG: «Текущий отпечаток
    #     журнала: `<64-hex>`»), — это значение хэша, а НЕ приватный ключ. Контекст
    #     задан кириллицей, и до PTD-0115 он не распознавался → ложное срабатывание.
    scenario("русский «отпечаток журнала: <64-hex>» в CHANGELOG — не утечка",
             lambda t: add_tracked(t, "CHANGELOG.md",
                                   "Всего записей: **115**. Текущий отпечаток журнала: `"
                                   + "a" * 64 + "`.\n"),
             expect_green=True)

    # 13. Слово «хэш»/«хеш» как русская подсказка тоже снимает ложную тревогу.
    scenario("русский «хэш коммита <64-hex>» — не утечка",
             lambda t: add_tracked(t, "REPORT.md",
                                   "хэш коммита " + "b" * 64 + "\n"),
             expect_green=True)

    # 14. Регрессия в обратную сторону: настоящий 64-hex без всякого хэш-контекста
    #     (ни ASCII, ни кириллица) Guardian обязан поднять в красный — расширение
    #     словаря на кириллицу не должно ослабить детектор.
    scenario("голый 64-hex без хэш-контекста — по-прежнему утечка",
             lambda t: add_tracked(t, "leak.txt", "c" * 64 + "\n"),
             expect_green=False, expect_fail_key="no-key-material")
