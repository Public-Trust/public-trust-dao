...
# Словесный токен в префиксе строки. Включает кириллицу: человекочитаемые тексты
# (CHANGELOG, отчёты) подписывают хэш русскими словами «отпечаток»/«хэш», и без
# кириллицы такой контекст не распознавался — легитимный отпечаток журнала ложно
# принимался за приватный ключ (PTD-0115).
...
# а русские подсказки («отпечат…», «хэш»/«хеш») — человекочитаемые ленты вроде CHANGELOG.
HASH_HINTS = ("hash", "sha", "cid", "fingerprint", "digest", "checksum", "commit",
              "integrity", "отпечат", "хэш", "хеш")
