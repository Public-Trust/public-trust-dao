...
| `mirror-doc-coverage` *(soft)* | the inverse of `mirror-doc-link`: every explanation screen in the "Understand how the fund works" showcase (`t.learn` in `lib/i18n.ts`) is registered in the `MIRROR_DOCS` map — if a new mirror screen is added but left out of the map, its retelling goes unchecked against the source, so it warns | Art. 3/6 — verifiability/clarity (`PTD-0109`) |
| `mirror-doc-exists` *(soft)* | the path value in the `MIRROR_DOCS` map (`docs/X.md`) really exists in the repo — if a normative document is renamed/moved but the map path is left stale, the mirror screen's link silently goes nowhere, and `mirror-doc-link` (which only checks the string on the screen) misses it | Art. 3/6 — verifiability/clarity (`PTD-0110`) |
| `mirror-doc-showcase` *(soft)* | the pair of `mirror-doc-coverage`: a built mirror screen from `MIRROR_DOCS` (its page exists) is shown in the `t.learn` showcase — otherwise the built explanation screen is "hidden" from a person past the home page | Art. 3/6 — verifiability/clarity (`PTD-0110`) |
| `mirror-doc-distinct` *(soft)* | each mirror screen has its own separate document: two different `MIRROR_DOCS` keys do not point to the SAME `docs/X.md` — otherwise one document gets two mirrors while another is left with none at all (almost always a typo in the map) | Art. 3/6 — verifiability/clarity (`PTD-0111`) |
...
| `mirror-doc-learn-slug` *(soft)* | the mirror of `mirror-doc-slug` from the other side: every address in the `t.learn` showcase itself (`lib/i18n.ts`) is a valid slug of exactly the form `/<lowercase-with-hyphens>/` (leading and trailing slash, only `a-z`, digits and single hyphens between them) — otherwise a malformed showcase address (`/Manifesto/`, `/direct help/`) silently drifts out of sync with the `MIRROR_DOCS` map, and `mirror-doc-slug` (which only inspects keys) never catches it | Art. 3/6 — verifiability/clarity (`PTD-0117`) |
| `mirror-doc-set-match` *(soft)* | a direct comparison of two slug sets: the `t.learn` showcase addresses and the `MIRROR_DOCS` map keys describe the same set of screens. Shows at once what exists only in the showcase and only in the map — the drift is explained in a single line instead of subtracting the `mirror-doc-coverage`/`mirror-doc-showcase` reports by hand. A map key without a built page is left out of the map-side comparison ("not done yet", not "drifted") | Art. 3/6 — verifiability/clarity (`PTD-0118`) |
| `mirror-doc-learn-built` *(soft)* | every `/slug/` address in the `t.learn` showcase (`lib/i18n.ts`) actually has a built screen page `platform/app/<slug>/page.tsx`. Closes a gap: `mirror-doc-coverage`/`mirror-doc-set-match` check the address against the map, and `link-integrity` only inspects `.md` — nobody caught "the address is in the showcase but the screen itself doesn't exist yet", so a person would click from the home page and hit a 404. No `i18n.ts` or platform directory → stays silent | Art. 3/6 — verifiability/clarity (`PTD-0119`) |
