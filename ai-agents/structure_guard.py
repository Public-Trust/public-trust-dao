...
слове» (никакая автоматика их не стерегла):
  ...
  4. CI-воркфлоу реально зовёт `run_all.py --with-tests`. Полные списки
     `AGENTS`/`TESTS` бесполезны, если сам файл `.github/workflows/ai-agents.yml`
     не запускает мета-агента (или запускает без `--with-tests`). Тогда покрытие
     обходится уже на уровне CI: проверка #3 «зелёная», а в сборке ничего не
     прогоняется. Эта проверка смотрит сам воркфлоу и закрывает последний обход.

  5. Триггер-пути воркфлоу включают `ai-agents/**`. Даже если воркфлоу зовёт
     ...
     триггера, чем на уровне команды (#4). Эта проверка смотрит сами блоки путей.

  6. CI-воркфлоу прогоняет тест-инвариант мета-агента `test_run_all.py`. Проверка
     #4 стережёт, что воркфлоу зовёт `run_all.py --with-tests` — то есть всех
     агентов и их тесты. Но сам мета-агент сводит частные «красное» в общий
     вердикт, и эта сводка могла бы тихо сломаться (рапортовать «зелено», когда
     один из агентов красный). Что сводка исправна, доказывает СОБСТВЕННЫЙ
     тест-инвариант `run_all` — `test_run_all.py`. Если воркфлоу не запускает его
     отдельным шагом, поломку сведения никто не поймает. Эта проверка требует,
     чтобы воркфлоу прогонял и `test_run_all.py`, а не только `run_all.py`.
...
def check_ci_calls_run_all(agents_dir):
    """CI-воркфлоу обязан реально звать run_all.py --with-tests (а не отдельные агент-команды).

    run-all-covers-all сторожит СПИСКИ AGENTS/TESTS внутри run_all.py. Но даже
    при полных списках покрытие можно обойти на уровне CI: если воркфлоу зовёт
    отдельные agent-команды (или run_all без --with-tests), мета-агент с его
    списками просто не исполняется, и тест-инварианты не прогоняются. Эта
    проверка закрывает последний обход: смотрит сам файл воркфлоу.

    Воркфлоу ищем от КОРНЯ репозитория (родитель каталога агентов). Его отсутствие
    здесь — нарушение (некому прогонять run_all), в отличие от отсутствия самого
    run_all.py в run-all-covers-all (там это громко валит другие шаги CI само)."""
    ...
    if not os.path.isfile(wf_path):
        return [{
            ...
            "problem": "CI-воркфлоу не найден — некому прогонять run_all (покрытие обходится на уровне CI)",
        }]
    try:
    ...
    # Строки, зовущие именно мета-агента run_all.py. `\b` перед `run_all` не даёт
    # спутать его с test_run_all.py (там перед 'run_all' стоит словесный символ).
    calls = [ln for ln in text.splitlines() if re.search(r"\brun_all\.py\b", ln)]
    if not calls:
        return [{
            "item": CI_WORKFLOW_REL,
            "problem": "CI не зовёт run_all.py — агенты прогоняются в обход общего мета-агента",
        }]
    if not any("--with-tests" in ln for ln in calls):
        return [{
            "item": CI_WORKFLOW_REL,
            "problem": "run_all.py зовётся без --with-tests — тест-инварианты не прогоняются в CI",
        }]
    return []
...
def check_trigger_paths_include_agents(agents_dir):
    ...
    ci-calls-run-all сторожит, что воркфлоу ЗОВЁТ мета-агента. Но вызов
    бесполезен, если воркфлоу не СРАБАТЫВАЕТ на правку агента: без `ai-agents/**`
...
def check_ci_runs_test_run_all(agents_dir):
    """CI-воркфлоу обязан отдельным шагом прогонять test_run_all.py.

    ci-calls-run-all сторожит, что воркфлоу зовёт run_all.py --with-tests — всех
    агентов и их тесты. Но сам мета-агент СВОДИТ частные «красное» в общий
    вердикт, и эта сводка могла бы тихо сломаться (рапортовать «зелено» при
    красном агенте). Что сводка исправна — доказывает собственный тест-инвариант
    run_all, файл test_run_all.py. Если воркфлоу не запускает его, поломку
    сведения никто не поймает. Здесь требуем прогон именно test_run_all.py.

    `\\btest_run_all\\.py\\b` не путается с вызовом самого run_all.py: ci-calls-run-all
    ищет `\\brun_all\\.py\\b`, где перед 'run_all' стоит словесный символ '_', так
    что эти две проверки смотрят на разные команды."""
    repo_root = os.path.dirname(os.path.abspath(agents_dir.rstrip("/")))
    wf_path = os.path.join(repo_root, CI_WORKFLOW_REL)
    if not os.path.isfile(wf_path):
        return [{
            "item": CI_WORKFLOW_REL,
            "problem": "CI-воркфлоу не найден — некому прогнать test_run_all.py (исправность сведения не доказана)",
        }]
    try:
        with open(wf_path, "r", encoding="utf-8") as fh:
            text = fh.read()
    ...
    if not any(re.search(r"\btest_run_all\.py\b", ln) for ln in text.splitlines()):
        return [{
            "item": CI_WORKFLOW_REL,
            "problem": "CI не зовёт test_run_all.py — исправность сведения run_all (красное→красное) не проверяется",
        }]
    return []
...
CHECKS = (
    ...
    {
        "key": "ci-calls-run-all",
        "title": "CI-воркфлоу реально зовёт run_all.py --with-tests",
        "guards": "ст. 3 — покрытие нельзя обойти и на уровне CI: воркфлоу прогоняет мета-агента с тестами",
        "fn": check_ci_calls_run_all,
    },
    {
    ...
    {
        "key": "ci-runs-test-run-all",
        "title": "CI-воркфлоу прогоняет тест-инвариант мета-агента test_run_all.py",
        "guards": "ст. 3 — исправность сведения run_all (красное→красное) проверяется, а не держится на дисциплине воркфлоу",
        "fn": check_ci_runs_test_run_all,
    },
)
