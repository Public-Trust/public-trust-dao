...
слове» (никакая автоматика их не стерегла):
  ...
  10. Метки форм обращений описаны в каталоге `.github/labels.yml`. Формы
     `.github/ISSUE_TEMPLATE/*.yml` автоматически вешают на новое обращение
     метки из поля `labels:` (`bug`/`idea`/`governance`/`safety`). Раньше цвет и
     описание этих меток жили только в настройках GitHub и заводились вручную —
     невоспроизводимо. Теперь источник правды — открытый файл `labels.yml`, а
     страж краснеет, если форма ссылается на метку, которой в каталоге нет
     (заведена руками/опечатка). Источник правды держится полным (ст. 3).
...
# Каталог форм обращений (issue) и единый каталог меток (от корня репозитория).
# Каждая метка, на которую ССЫЛАЕТСЯ форма обращения (`labels: [...]`), обязана
# быть описана в labels.yml — иначе цвет/описание метки заводятся вручную и
# невоспроизводимы (а форма «вешает» метку, которой нет в источнике правды).
ISSUE_TEMPLATE_REL = os.path.join(".github", "ISSUE_TEMPLATE")
LABELS_FILE_REL = os.path.join(".github", "labels.yml")
...
def _strip_yaml_scalar(value):
    """Снимает обрамляющие кавычки и хвостовой комментарий с простого YAML-скаляра."""
    value = value.strip()
    # хвостовой комментарий только вне кавычек
    if value[:1] not in ("'", '"'):
...
def _issue_form_labels(text):
    """Имена меток, на которые ссылается форма обращения через ключ `labels:`.

    Поддержаны обе формы записи: инлайн-список `labels: ["bug", "x"]` (как сейчас
    в наших формах) и блочный список под ключом. Возвращает множество имён меток."""
    labels = set()
    lines = text.splitlines()
    for idx, line in enumerate(lines):
        ...
        if rest.startswith("["):
            ...
            for part in inner.split(","):
                name = _strip_yaml_scalar(part)
                if name:
                    labels.add(name)
        elif rest and not rest.startswith("#"):
            # скаляр в одну строку (необычно, но допустимо)
            name = _strip_yaml_scalar(rest)
            if name:
                labels.add(name)
        else:
            # блочный список: последующие строки '  - name'
            for follow in lines[idx + 1:]:
                ...
                if not item:
                    if follow.strip() == "":
                        continue
                    break
                name = _strip_yaml_scalar(item.group(1))
                if name:
                    labels.add(name)
    return labels


def _defined_label_names(text):
    """Имена меток, определённых в .github/labels.yml (строки `- name: ...`)."""
    names = set()
    for m in re.finditer(r"(?m)^-\s*name:\s*(.+)$", text):
...
def check_issue_form_labels_defined(agents_dir):
    """Каждая метка, на которую ссылается форма обращения, описана в labels.yml.

    Формы `.github/ISSUE_TEMPLATE/*.yml` автоматически вешают на новое обращение
    метки из своего поля `labels:` (`bug` / `idea` / `governance` / `safety`).
    Раньше цвет и описание этих меток жили только в настройках GitHub и заводились
    вручную — их нельзя было восстановить или проверить. Теперь источник правды —
    `.github/labels.yml`. Эта проверка краснеет, если форма ссылается на метку,
    которой НЕТ в каталоге: тогда метка либо заведена руками (невоспроизводимо),
    либо у неё опечатка — и каталог перестаёт быть полным источником правды.

    Нарушение называет ровно недостающую метку и форму, где она встретилась.
    Если каталога меток нет вовсе, но формы ссылаются на метки — это тоже
    нарушение (источник правды отсутствует)."""
    repo_root = os.path.dirname(os.path.abspath(agents_dir.rstrip("/")))
    forms_dir = os.path.join(repo_root, ISSUE_TEMPLATE_REL)
    labels_path = os.path.join(repo_root, LABELS_FILE_REL)

    # Собираем метки, на которые ссылаются формы (config.yml поля labels не имеет).
    referenced = {}  # имя метки -> имя файла-формы, где впервые встретилась
    if os.path.isdir(forms_dir):
        for fname in sorted(os.listdir(forms_dir)):
            if not (fname.endswith(".yml") or fname.endswith(".yaml")):
                continue
            try:
                with open(os.path.join(forms_dir, fname), "r", encoding="utf-8") as fh:
                    form_text = fh.read()
            except OSError:
                continue
            for name in _issue_form_labels(form_text):
                referenced.setdefault(name, fname)

    if not referenced:
        # Формы не вешают меток — проверять нечего (каталог может быть пустым/отсутствовать).
        return []

    if not os.path.isfile(labels_path):
        return [{
            "item": LABELS_FILE_REL,
            "problem": "формы обращений вешают метки ({}), но каталог меток отсутствует — нет источника правды для цветов/описаний".format(
                ", ".join(sorted(referenced))),
        }]

    try:
        with open(labels_path, "r", encoding="utf-8") as fh:
            defined = _defined_label_names(fh.read())
    except OSError as exc:
        return [{"item": LABELS_FILE_REL, "problem": "не удалось прочитать каталог меток: {}".format(exc)}]

    violations = []
    for name in sorted(referenced):
        if name not in defined:
            violations.append({
                "item": "{} → метка '{}'".format(referenced[name], name),
                "problem": "метка не описана в {} — заводится вручную/опечатка; добавь её в каталог меток".format(LABELS_FILE_REL),
            })
    return violations
...
CHECKS = (
    ...
    {
        "key": "issue-form-labels-defined",
        "title": "Метки форм обращений описаны в каталоге labels.yml",
        "guards": "ст. 3 — цвета/описания меток воспроизводимы из одного открытого источника, а не заводятся вручную",
        "fn": check_issue_form_labels_defined,
    },
)
