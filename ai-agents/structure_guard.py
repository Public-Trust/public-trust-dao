...
def _workflow_run_steps(text):
    """Возвращает тела всех шагов `run:` воркфлоу (в порядке появления).

    Тело — это значение ключа `run:`: либо инлайн (`run: cmd` / `- run: cmd`),
    либо блочный скаляр (`run: |` с последующими строками глубже отступа ключа).
    Берём ИМЕННО тела команд, а не весь файл: команда, упомянутая лишь в
    комментарии или в названии шага (`- name: ... run_all ...`), сюда не попадёт —
    значит она не запускается, и страж это заметит. Разбор текстовый, без
    сторонних зависимостей (достаточно для блочного YAML наших воркфлоу)."""
    lines = text.splitlines()
    steps = []
    i = 0
    n = len(lines)
