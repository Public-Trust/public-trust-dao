...
- [`docs/en/PROOF-OF-CONTRIBUTION.md`](docs/en/PROOF-OF-CONTRIBUTION.md) — proof of contribution and escrow contracts: the fund creates paid work through helping people; how a contract provably confirms execution (recipient signature + artifact hashes + independent validators staking reputation) before paying; honest about the oracle problem.
- [`docs/en/IDENTITY-VERIFICATION.md`](docs/en/IDENTITY-VERIFICATION.md) — making sure a real human is behind an account, without surveillance: one human = one account; the fund collects no face database (an external service does the check, only a "verified" mark stays with us); on-chain — only facts, no personal data; proof without revealing identity; ready-made proof-of-personhood options to choose from; a mandatory no-camera fallback (in-person vouching); strictness by risk.
- [`docs/en/ACCOUNTABILITY.md`](docs/en/ACCOUNTABILITY.md) — the "everything signed and traceable" principle: every meaningful action (of a member, AI, or developer) has an author, is signed, and is recorded tamper-evidently; money/power/agreements are additionally anchored by a fingerprint on-chain (personal data is not); AI is always under signature but holds no sole power.
